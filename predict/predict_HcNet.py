...
def loss_adc_acc():
    df = pd.read_csv(r'/remote-home/hongzhangxin/pytorch_project/TB_Classfiy/result/loss_acc_HcNet.csv')
    losses = df["loss"].values
    accuracies = df["acc"].values
    plt.title("HcNet_acc_loss")
    ...
    plt.savefig("/remote-home/hongzhangxin/pytorch_project/TB_Classfiy/reslut_img/acc_loss_HcNet.png")
    plt.show()
    plt.close()
...
class ConfusionMatrix(object):
    ...
    def plot(self):
        ...
        plt.title('HcNet Confusion matrix')
        # 在图中标注数量/概率信息
        thresh = matirx.max() / 2
        ...
        plt.tight_layout()
        plt.savefig('/remote-home/hongzhangxin/pytorch_project/TB_Classfiy/reslut_img/HcNet_ConfusionMatrix.png')
        plt.show()
        plt.close()
...
if __name__ == '__main__':
    ...
    # 加载模型 加载训练好的参数
    model = HcNet(num_classes=2)
    model_weight_path = os.path.join(paras.save_path, 'HcNet.pth')
